...
3. Build movie effects model
4. Build movie and user effects model
...
First of all, load the data and packages.   

```{r load the dataset, cache=TRUE}
...
```{r grance at data structure}
glimpse(edx)

glimpse(validation)
```

We can see that how many obsevations and variables are in the datasets, and also see that what data types they are. 
...
We can see that 4 is most counted and 3 is the second. Form histgram, average rating would fall on the between 3 and 4. 
...
### 2.3. Create movie effects model
...
$$
We estimate $\hat{b_i}$ by taking average of $Y_{u,i} - \mu$ for each movie $i$. Let's see the histgram of $b_i$.
...
We can confirm that movie bias is exist from this histgram. Thus, our movie effects model should work. Let's predict rating and calculate RMSE.  
...
### 2.4. create movie and user effects model

As well as movie effects, there would be user effects. To confirm that, let's see the histgram of the average rating for user $u$ for those that have rated over 100 movies:  
